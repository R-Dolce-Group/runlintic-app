...
function showPostInstallHelp() {
  ...
  // Check if we're in a potential project directory - atomic file checks
  ...
  try {
    fs.accessSync(path.join(cwd, 'turbo.json'), fs.constants.R_OK);
    hasTurboJson = true;
  } catch {
    // turbo.json doesn't exist or isn't readable
    hasTurboJson = false;
  }
  ...
  console.log('⬆️  Upgrade options: https://rdolcegroup.com/runlintic-app\n');
}
