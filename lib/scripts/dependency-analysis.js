...
class DependencyAnalyzer {
  ...
  async runUpdatesAnalysis() {
    console.log(`${COLORS.cyan}Analyzing available package updates...${COLORS.reset}`);

    const rawOutdated = await this.analyzeOutdatedPackages();
    const security = { vulnerabilities: [] }; // Empty security for updates focus
    const recommendations = this.generateUpdateRecommendations(rawOutdated, security);

    const compatibility = await this.assessCompatibilityRisks(recommendations);

    const analysisResults = {
      outdated: rawOutdated, // Keep raw outdated for compatibility
      unused: { dependencies: [], devDependencies: [] }, // Skip unused for updates focus
      security: security,
      recommendations: recommendations, // Add processed recommendations
      compatibility: compatibility, // Add compatibility assessment
      summary: {}
    };

    analysisResults.summary = {
      totalPackages: Object.keys({ ...this.packageJson.dependencies, ...this.packageJson.devDependencies }).length,
      updatesAvailable: recommendations.length,
      unusedDeps: 0,
      securityIssues: 0
    };
  ...
  async runSecurityAnalysis() {
    console.log(`${COLORS.cyan}Running security-focused dependency analysis...${COLORS.reset}`);

    const rawOutdated = await this.analyzeOutdatedPackages();
    const security = await this.analyzeSecurityVulnerabilities();
    const allRecommendations = this.generateUpdateRecommendations(rawOutdated, security);

    // Only include security-related updates (CRITICAL or HIGH risk)
    const securityRecommendations = allRecommendations.filter(pkg =>
      pkg.riskLevel === RISK_LEVELS.CRITICAL.level ||
      pkg.riskLevel === RISK_LEVELS.HIGH.level
    );

    const compatibility = await this.assessCompatibilityRisks(securityRecommendations);

    const analysisResults = {
      outdated: rawOutdated, // Keep raw outdated for compatibility
      unused: { dependencies: [], devDependencies: [] }, // Skip unused for security focus
      security: security,
      recommendations: securityRecommendations, // Add filtered security recommendations
      compatibility: compatibility, // Add compatibility assessment
      summary: {}
    };
    ...
    analysisResults.summary = {
      totalPackages: Object.keys({ ...this.packageJson.dependencies, ...this.packageJson.devDependencies }).length,
      updatesAvailable: securityRecommendations.length,
      unusedDeps: 0,
      securityIssues: security.vulnerabilities ? Object.keys(security.vulnerabilities).length : 0
    };

    return analysisResults;
  }

  getExitCode(analysisResults) {
    // CI/CD integration - return non-zero exit codes for issues
    const { security, recommendations } = analysisResults;

    // Critical security vulnerabilities = exit code 2
    const vulnerabilityCount = security.vulnerabilities ? Object.keys(security.vulnerabilities).length : 0;
    if (vulnerabilityCount > 0) {
      return 2;
    }

    // High-risk updates available = exit code 1
    const highRiskUpdates = recommendations ? recommendations.filter(pkg => pkg.riskLevel === RISK_LEVELS.HIGH.level) : [];
    if (highRiskUpdates.length > 0 && this.options.focus === 'security') {
      return 1;
    }
  ...
  async run() {
    try {
      ...
      // Handle focused analysis modes
      if (this.options.focus === 'updates') {
        analysisResults = await this.runUpdatesAnalysis();
      } else if (this.options.focus === 'security') {
        analysisResults = await this.runSecurityAnalysis();
      } else {
        analysisResults = await this.runComprehensiveAnalysis();
      }

      // Set exit code based on findings for CI/CD integration
      const exitCode = this.getExitCode(analysisResults);
