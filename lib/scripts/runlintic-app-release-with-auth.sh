...
# Configure Git to use token-based authentication (safer approach)
...
# Run the release command using locally installed release-it in non-interactive mode
...
# Run release-it with timeout to prevent hanging
...
# For now, run without timeout until we implement OS-independent solution
