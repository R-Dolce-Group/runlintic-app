...
if [[ -n "${GH_TOKEN:-}" && -n "${NPM_ACCESS_TOKEN:-}" ]]; then
  ...
  # Handle both NPM_TOKEN and NPM_ACCESS_TOKEN naming
  ...
  echo "💡 Example: export GH_TOKEN=your_github_token" >&2
  echo "💡 Example: export NPM_TOKEN=your_npm_token" >&2
  exit 1
fi
...
if [[ -z "${GH_TOKEN:-}" ]]; then
  ...
  echo "💡 Set as environment variable" >&2
  exit 1
fi
...
if [[ -z "${NPM_ACCESS_TOKEN:-}" ]]; then
  ...
  echo "💡 Set NPM_TOKEN as environment variable" >&2
  exit 1
fi
